...
*/
func LoopForTestStrings(t *testing.T, tf func(Parameters) string, tests []TCStrings) {
	t.Helper()
...
func SetupRNG(params Parameters) *rand.Rand {
	var rs rand.Source
	var rng *rand.Rand
	if s := params.Seed(); s > 0 {
		rs = rand.NewSource(s)
	} else {
...
//NewParameters - new Parameters struct
func (p Parameters) NewParameters(s int64) Parameters {
	ret := Parameters{s}
	return ret
}

//Seed - returns the seed
func (p Parameters) Seed() int64 {
	return p.seed
}
